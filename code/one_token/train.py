...
DATA_FILE = "/root/autodl-tmp/PAUE-II/benchmark/reprocess/train/train_anony_with_loss.jsonl" 
CKPT_DIR = "/root/autodl-tmp/PAUE-II/ckpts"   
...
class TrainableEnhancer(torch.nn.Module):
    ...
    def __init__(self, uem_path: str, uem_tokenizer, llm_model, llm_tokenizer):
        ...
        self.projection_layer = torch.nn.Sequential(
            ...
            torch.nn.Linear(self.uem.config.hidden_size * 4, self.llm_frozen.config.hidden_size)
        )
    ...
    def forward(self, 
        ...
        # 3. 将前缀嵌入移动到 LLM 的设备上
        prefix_embeds = clipped_prefix.unsqueeze(1).to(device=self.llm_frozen.device, dtype=self.llm_frozen.dtype)
...
def main():
    ...
    # 加载数据集并拆分
    full_dataset = GetDataset(DATA_FILE)
